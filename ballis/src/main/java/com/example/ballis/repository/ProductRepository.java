...
public interface ProductRepository extends JpaRepository<Product, Long> {
	...
	@Query(value="SELECT "
	          ...
	          + "(prod.id, prod.productEngName, img.imagePath, bd.brandName, sell.wishPrice) "
	            ...
	            + "LEFT JOIN Selling sell ON prod.id = sell.product AND NOT sell.sellingStatus = 99 "
	            + "WHERE img.pageDiv = 1 AND img.mainImageDiv = 1 "
	            + "AND (sell.wishPrice IS NULL OR "
	            + "sell.wishPrice = ("
	            + "SELECT MIN(wishPrice) "
	            + "FROM Selling "
	            + "WHERE product = prod.id AND NOT sellingStatus = 99)) "
...
//	
	...
	@Query(value="SELECT "
	          ...
	          + "buy.buyingStatus, buy.productSize, MAX(buy.wishPrice), buy.member.memberNumber) "
	            + "FROM Product prod "
	            + "INNER JOIN Image img ON prod.id = img.targetId "
	            + "LEFT JOIN Selling sell ON prod.id = sell.product AND (sell.sellingStatus = 1 OR sell.sellingStatus = 11) "
	            ...
	            + "WHERE prod.id = :productid "
	            + "AND img.pageDiv = 1 AND img.mainImageDiv = 1 "
